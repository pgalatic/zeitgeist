...
def create(target, summary, cluster_reps, sent_reps, seed=None, label=None):
    ...
    sloc_1 =        (int(width * 0.02), int(height * 0.40))
    cluster_0_loc = (int(width * 0.02), int(height * 0.43))
    cluster_1_loc = (int(width * 0.34) + BUFFER // 4, int(height * 0.43))
    cluster_2_loc = (int(width * 0.66) + BUFFER // 2, int(height * 0.43))
    sloc_2 =        (int(width * 0.02), int(height * 0.60))
    sent_0_loc =    (int(width * 0.02), int(height * 0.63))
    sent_1_loc =    (int(width * 0.34) + BUFFER // 4, int(height * 0.63))
    sent_2_loc =    (int(width * 0.66) + BUFFER // 2, int(height * 0.63))
    sloc_3 =        (int(width * 0.02), int(height * 0.80))
    ...
    graph_size = (width - (graph_loc[0]*2), sloc_1[1] - (graph_loc[1] + SPACING))
    sloc_size = (width - sloc_1[0]*2, cluster_0_loc[1] - (sloc_1[1] + SPACING))
    cluster_size = (cluster_1_loc[0] - (cluster_0_loc[0] + SPACING), sloc_2[1] - (cluster_0_loc[1] + SPACING))
    sent_size = (sent_1_loc[0] - (sent_0_loc[0] + SPACING), height - (sent_4_loc[1] + SPACING))
    ...
    slocbox_1 = slocbox(sloc_size, 'These tweets represent factions who each use the same words.')
    slocbox_2 = slocbox(sloc_size, 'These tweets represent factions who feel strongly on this subject.')
    slocbox_3 = slocbox(sloc_size, 'These tweets represent the largest factions who all feel similarly.')
    
    img.paste(summary_img, summary_loc, summary_img)
    img.paste(graph_img, graph_loc, graph_img)
    img.paste(slocbox_1, sloc_1, slocbox_1)
    ...
    img.paste(slocbox_2, sloc_2, slocbox_2)
    ...
    img.paste(slocbox_3, sloc_3, slocbox_3)
